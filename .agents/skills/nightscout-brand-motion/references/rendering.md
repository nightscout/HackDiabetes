...
node render-anim.mjs --in ../NS-endcard.svg --out ../out/NSF-endcard --fps 30 --width 1920 --height 1080
```

`out/` is git-ignored: rendered files are **regenerated on demand**, never tracked
(no frame sequences, no ProRes masters, no format zoo in the repo).

## Formats

Default is **`prores,mp4`** (alpha master + flat preview). Ask for more via
`--formats`:

| format | file | alpha | use |
|--------|------|:---:|-----|
| `prores` *(default)* | `.mov` ProRes 4444 | ✅ | Premiere / FCP / Resolve master |
| `mp4` *(default)* | `.mp4` H.264 on `--mp4-bg` | ❌ | universal preview |
